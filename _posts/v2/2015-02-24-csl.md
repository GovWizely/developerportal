...
Searches only entities whose **country** fields match the country code based on [ISO alpha-2 country codes](http://www.iso.org/iso/home/standards/country_codes/country_names_and_code_elements.htm). The **country** fields are found in the **address** and **ids** arrays.This method allows you to search for multiple countries (plural) separated by commas but will only return one country (singular) per entity.  
